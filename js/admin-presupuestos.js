...
// muestra un mensaje de éxito, error o confirmación
...
function actualizarPresupuesto(
  ...
  mostrarMensaje("error", "Error al actualizar el presupuesto");
  return null;
}
...
function listarPresupuestos() {
  ...
  presupuestos.forEach((presupuesto) => {
    ...
    const servicios = obtenerServicios()
      .filter((s) => presupuesto.servicios.includes(s.id))
      .map((s) => s.nombre)
      .join(", ");
    html += `
        <tr>
            ...
            <td>
                <button class="btn btn-sm btn-warning" onclick="cargarVistaPresupuesto('presupuestos', 'editar', ${
                  presupuesto.id
                })">Editar</button>
  ...
  if (presupuestos.length === 0) {
    html += `<tr><td colspan="13" class="text-center">Sin presupuestos registrados</td></tr>`;
  }
  html += `</tbody></table>`;
...
function mostrarFormularioCrearPresupuesto(presupuesto = null) {
  ...
  const formHtml = `
    ...
    <form id="form-presupuesto" class="needs-validation" novalidate>
        ...
        <div class="mb-3">
            ...
            <div class="invalid-feedback">Selecciona un salón</div>
        </div>
        ...
        <div class="mb-3">
            ...
            <select multiple class="form-select" id="servicios" required>
                ...
                <option value="${servicio.id}" ${
                      esEdicion && presupuesto.servicios.includes(servicio.id) ? "selected" : ""
                    }>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="estado" required ${esEdicion ? "" : "disabled"}>
                        <option value="pendiente" ${
                          esEdicion && presupuesto.estado === "pendiente" ? "selected" : ""
                        }>Pendiente</option>
                        <option value="aprobado" ${
                          esEdicion && presupuesto.estado === "aprobado" ? "selected" : ""
                        }>Aprobado</option>
                        <option value="rechazado" ${
                          esEdicion && presupuesto.estado === "rechazado" ? "selected" : ""
                        }>Rechazado</option>
                    </select>
  ...
  form.addEventListener("submit", function (e) {
    ...
    const serviciosSeleccionados = Array.from(document.getElementById("servicios").selectedOptions).map((option) =>
...
function eliminarPresupuesto(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      guardarPresupuestos(presupuestos);
      listarPresupuestos();
      mostrarMensaje("success", "Presupuesto eliminado correctamente");
    } else {
      Swal.fire("Cancelado", "El presupuesto no fue eliminado", "error");
    }
  });
}
...
function renderizarPresupuestos() {
        ...
        <div class="col">
            <div class="card h-100 shadow">
                <div class="card-body">
                    ...
                    <button class="btn btn-sm btn-warning" onclick="cargarVistaPresupuesto('presupuestos', 'editar', ${parseInt(
                      presupuesto.id
                    )})">Editar</button>
                    <button class="btn btn-sm btn-danger" onclick="eliminarPresupuesto(${
                      presupuesto.id
                    })">Eliminar</button>
                </div>
            </div>
  ...
  if (presupuestos.length === 0) {
    contenedor.innerHTML = `<p class="text-center">Sin presupuestos registrados</p>`;
  }
}
